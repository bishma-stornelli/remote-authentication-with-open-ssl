...
void parse_options(int argc, char ** argv, struct sockaddr_in *servaddr){
  ...
  while ((option = getopt (argc, argv, "s:p:")) != -1){
    switch (option) {
      case 's':
        if ( inet_aton(optarg, &servaddr->sin_addr) <= 0 ) {
  ...
  if( args_provided != 2 ){
    fprintf(stderr, "You must specify IP address and port number with the next syntax:\n\n");
    fprintf(stderr, "\t./program -s ip-server -p port-server\n\n");
    exit(EXIT_FAILURE);
  }
}
